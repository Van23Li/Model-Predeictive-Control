...
while 1
    F = O.H(:,1:2);
    f = O.H(:,3);
    Opre = projection(Polyhedron([F*A F*B; zeros(2) Hu],[f;hu]), 1:2);
	h2_1 = plot(Opre, 'color', 'y', 'alpha', 0.1);
    Opre = intersect(O, Opre);
...
h3=plot(O,'color', 'g');
legend([h3;h1;h2_2],{'Control Invariant set';'State constraints';'Iterations'});
