...
for mu = logspace(-3,-0.1,50)
    ...
    display = 0;
    [prob,opt] = setupEx2(dim, speed, seed, display);
    ...
    while kappa > opt.epsilon % Stop once the barrier term is small
        ...
        while 1
            ...
            Dz = inv(prob.H + kappa * prob.grad2Phi(z)) * ...
                (-prob.H * z - prob.q - kappa * prob.gradPhi(z));
            
            %^^^^^^^^^^^^^^^^ YOUR CODE HERE ^^^^^^^^^^^^^^^^^^^
            
            if dim == 2 && display
                plot([z(1);zprev(1)],[z(2);zprev(2)],'k.-','markersize',10);
                drawnow;
            end
        ...
        if dim == 2 && display, plot(z(1),z(2),'ro'); drawnow; end
    ...
    steps_list = [steps_list, stats.innerIterations];
end

figure(2)
plot(mu_list, steps_list, '*-');
end

function [prob, opt] = setupEx2(dim, speed, seed, display)
%
...
% prob.gradPhi = @(z) prob.G'*(1./(prob.d - prob.G * z));
prob.diag_G = [];
for i = 1:length(prob.d)
    prob.diag_G = blkdiag(prob.diag_G, prob.G(i,:));
end

prob.grad2Phi= @(z) prob.G' * prob.diag_G * ...
    kron(diag(1./(prob.d-prob.G*z).^2),eye(dim)) * ...
    repmat(eye(dim),length(prob.d),1);
end